 ...
 */
package com.yogurt.utils.common.codec;
...
public class EncodeUtils {
    ...
    private static Pattern p2 = Pattern.compile("\\s*on[a-z]+\\s*=\\s*(\"[^\"]+\"|'[^']+'|[^\\s]+)\\s*(?=>)", Pattern
            .CASE_INSENSITIVE);
    private static Pattern p3 = Pattern.compile("\\s*(href|src)\\s*=\\s*(\"\\s*(javascript|vbscript):[^\"]+\"|'\\s*" +
            "(javascript|vbscript):[^']+'|(javascript|vbscript):[^\\s]+)\\s*(?=>)", Pattern.CASE_INSENSITIVE);
     ...
     * 2、\s*on[a-z]+\s*=\s*("[^"]+"|'[^']+'|[^\s]+)\s*(?=>)
     * 3、\s*(href|src)\s*=\s*("\s*(javascript|vbscript):[^"]+"|'\s*(javascript|vbscript):[^']+'|(javascript|vbscript)
     * :[^\s]+)\s*(?=>)
     * 4、epression\((.|\n)*\);?
     * 其它情况下：进行HTML4编码
     *
    ...
    private static Pattern p5 = Pattern.compile("(?:')|(?:--)|(/\\*(?:.|[\\n\\r])*?\\*/)|(\\b" +
            "(select|update|and|or|delete|insert|trancate|char|into|substr|ascii|declare|exec|count|master|into|drop" +
            "|execute)\\b)", Pattern.CASE_INSENSITIVE);
